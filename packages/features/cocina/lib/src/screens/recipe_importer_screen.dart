...
class RecipeImporterScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<RecipeImporterScreen> createState() => _RecipeImporterScreenState();
}
...
class _RecipeImporterScreenState extends ConsumerState<RecipeImporterScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Chef IA', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
  ...
  Widget _buildContent(RecipeImportState state, RecipeImportNotifier notifier) {
    if (state == RecipeImportState.downloadingVideo || state == RecipeImportState.extractingAI) {
      return Center(
        child: Column(
        ...
        child: Column(
          ...
          children: [
            ...
            _GlassCard(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(recipe.name, style: const TextStyle(color: Color(0xFF00F0FF), fontSize: 20, fontWeight: FontWeight.bold)),
                    ...
                    Row(
                      children: [
                        ...
                        Text('\${recipe.durationMinutes} min', style: const TextStyle(color: Colors.white)),
                        const SizedBox(width: 16),
                        const Icon(Icons.restaurant, color: Colors.white70, size: 16),
                        const SizedBox(width: 4),
                        Text('\${recipe.servings} porciones', style: const TextStyle(color: Colors.white)),
                      ],
                    ),
                    const Divider(color: Colors.white24, height: 32),
                    const Text('Ingredientes detectados:', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    ...recipe.ingredients.map((ing) => Padding(
                      padding: const EdgeInsets.symmetric(vertical: 4.0),
                      child: Text('- \${ing.quantity} \${ing.unit} de \${ing.ingredientName}', style: const TextStyle(color: Colors.white70)),
                    )),
                  ],
                ),
              ),
            ...
            ElevatedButton(
              ...
              onPressed: () {
                // TODO: Save to Drift Repository
                Navigator.of(context).pop();
              },
              child: const Text('Guardar en mi recetario', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            ),
            TextButton(
              onPressed: notifier.reset,
              child: const Text('Empezar de nuevo', style: TextStyle(color: Colors.white54)),
            ),
          ],
        ),
      );
        ...
        children: [
          ...
          if (state == RecipeImportState.error) ...[
             Container(
               ...
               child: Text('Error: \${notifier.errorMessage}', style: const TextStyle(color: Colors.redAccent)),
             ),
             const SizedBox(height: 24),
          ],
          _GlassCard(
                  ...
                  ElevatedButton(
                    ...
                    child: const Row(
                      ...
                      children: [
                        ...
                        SizedBox(width: 8),
                        Text('Analizar Enlace', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                      ],
                    ),
                  ),
                ],
          ...
          const SizedBox(height: 24),
          const Center(child: Text('— O —', style: TextStyle(color: Colors.white54, fontWeight: FontWeight.bold))),
          const SizedBox(height: 24),
          OutlinedButton.icon(
             ...
             label: const Text('Subir video desde Galería', style: TextStyle(fontSize: 16)),
