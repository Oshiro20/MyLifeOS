...
class WardrobeTab extends ConsumerWidget with AppFeedback {
  ...
  static const _typeEmoji = {
    GarmentType.shirt: '👕', GarmentType.tshirt: '👚',
    GarmentType.pants: '👖', GarmentType.jeans: '🩱',
    GarmentType.shoes: '👟', GarmentType.jacket: '🧥',
    GarmentType.accessories: '💍', GarmentType.dress: '👗',
    GarmentType.shorts: '🩳', GarmentType.sweater: '🧣',
    GarmentType.hoodie: '🧥', GarmentType.skirt: '👗',
    GarmentType.other: '🎽',
  };
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final sortedTypes = groupedGarments.keys.toList()..sort((a,b) => a.index.compareTo(b.index));
          ...
          children: [
            if (total > 0)
              Container(
                ...
                child: Row(children: [
                  _StatBadge(icon: isShoesTab ? Icons.snowshoeing : Icons.checkroom, label: '$total ${isShoesTab ? 'pares' : 'prendas'}',
                      color: const Color(0xFF00C896)),
                  const SizedBox(width: 12),
                  if (dirty > 0)
                    _StatBadge(icon: Icons.local_laundry_service_outlined,
                        label: '$dirty para lavar', color: const Color(0xFFFFB74D)),
                ]),
              ),
            Expanded(
              child: filteredGarments.isEmpty
                  ...
                  : GridView.builder(
                      ...
                      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                        ...
                        crossAxisSpacing: 16, childAspectRatio: 1.1,
                      ),
                      ...
                      itemBuilder: (ctx, sectionIndex) {
                        ...
                        return InkWell(
                          onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => WardrobeCategoryScreen(type: type))),
                          ...
                          child: Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                Text(_typeEmoji[type] ?? '🎽', style: const TextStyle(fontSize: 40)),
                                const SizedBox(height: 12),
...
class _GarmentCard extends StatelessWidget {
  ...
  const _GarmentCard({
    required this.garment, required this.emoji,
    required this.onToggleClean, required this.onToggleFav, required this.onDelete,
  });

  Color get _color {
    try {
      return Color(int.parse('FF${garment.primaryColor.replaceAll('#', '')}', radix: 16));
    } catch (_) {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              _SmallBtn(
                icon: garment.isClean ? Icons.local_laundry_service : Icons.local_laundry_service_outlined,
                color: garment.isClean ? Colors.white24 : const Color(0xFFFFB74D),
                tooltip: garment.isClean ? 'Marcar sucia' : 'Marcar limpia',
                onTap: onToggleClean,
              ),
              _SmallBtn(
                icon: garment.isFavorite ? Icons.favorite : Icons.favorite_border,
                color: garment.isFavorite ? const Color(0xFFFF4D4D) : Colors.white24,
                tooltip: 'Favorito', onTap: onToggleFav,
              ),
              _SmallBtn(icon: Icons.delete_outline, color: Colors.white24,
                  tooltip: 'Eliminar', onTap: onDelete),
            ]),
          ]),
        ),
      ]),
...
class _SmallBtn extends StatelessWidget {
  ...
  const _SmallBtn({required this.icon, required this.color, required this.tooltip, required this.onTap});
  @override
...
class _AddGarmentSheetState extends ConsumerState<_AddGarmentSheet> {
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Expanded(
                  child: _field(_colorCtrl, 'Color hex (ej: #3A86FF)', Icons.color_lens_outlined,
                      errorText: _colorError, onChanged: (_) => setState(() => _colorError = null)),
                ),
                const SizedBox(width: 12),
                Container(
                  width: 48, height: 48,
            ...
            Builder(builder: (context) {
              final shoeTypes = [GarmentType.shoes, GarmentType.sneakers, GarmentType.boots, GarmentType.sandals];
              final items = widget.isShoesTab 
                  ? shoeTypes 
                  : GarmentType.values.where((t) => !shoeTypes.contains(t)).toList();
              
              return _dropdown<GarmentType>('Tipo', Icons.checkroom_outlined, _type,
                  items, (v) => v.label, (v) => setState(() => _type = v!));
            }),
            const SizedBox(height: 8),
            _dropdown<GarmentStyle>('Estilo', Icons.style_outlined, _style,
                GarmentStyle.values, (v) => v.label, (v) => setState(() => _style = v!)),
            const SizedBox(height: 8),
            _dropdown<Season>('Temporada', Icons.wb_sunny_outlined, _season,
                Season.values, (v) => v.label, (v) => setState(() => _season = v!)),
            const SizedBox(height: 8),
            ...
            const SizedBox(height: 8),
            _field(_brandCtrl, 'Marca (opcional)', Icons.branding_watermark_outlined),
            const SizedBox(height: 8),
            Row(children: [
              Expanded(child: _field(_sizeCtrl, 'Talla (opc)', Icons.straighten_outlined)),
              const SizedBox(width: 8),
              Expanded(child: _field(_priceCtrl, 'Precio (opc)', Icons.attach_money_outlined)),
            ]),
            const SizedBox(height: 8),
            Row(children: [
              const Text('Rating:', style: TextStyle(color: Colors.white, fontSize: 13)),
              const SizedBox(width: 8),
  ...
  Widget _buildPhotoBox({required File? file, required String label, required VoidCallback onTap, required VoidCallback onClear}) {
    return Stack(
      children: [
        InkWell(
          onTap: onTap,
          ...
          child: AspectRatio(
            ...
            child: Container(
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: const Color(0xFF00C896).withAlpha(80), width: 1.5),
              ),
              child: file != null
                ? ClipRRect(
                    borderRadius: BorderRadius.circular(12),
  ...
  Future<void> _takePhoto({bool isDetails = false}) async {
    ...
    if (isDetails) return; // No AI on the secondary picture (just save the path locally)

    // Inferencia con Gemini AI
    try {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Gemini analizando prenda...'), duration: Duration(seconds: 1)));
      ...
      if (jsonStr != null && mounted) {
        ...
        setState(() {
          ...
          _type = GarmentType.values[parseInt(decoded['typeIndex'], 1).clamp(0, GarmentType.values.length - 1)];
          ...
          _season = Season.values.firstWhere((e) => e.name == sc, orElse: () => Season.all);
          
          _style = GarmentStyle.values[parseInt(decoded['styleIndex'], 0).clamp(0, GarmentStyle.values.length - 1)];
          _autoDetected = true;
        });
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error AI Garment: $e'), backgroundColor: const Color(0xFFFF5252)));
    } finally {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final garment = WardrobeGarment(
        ...
        type: _type, primaryColor: _colorCtrl.text.trim(),
        style: _style, material: _matCtrl.text.trim(), season: _season,
      ...
      if ((_type == GarmentType.tshirt || _type == GarmentType.shirt) && _style == GarmentStyle.sport) {
        wantShort = await showDialog<bool>(
          ...
          builder: (ctx) => AlertDialog(
            ...
            actions: [
              TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('No por ahora', style: TextStyle(color: Colors.white38))),
              ElevatedButton(onPressed: () => Navigator.pop(ctx, true), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF00C896)), child: const Text('Sí, escanear', style: TextStyle(color: Colors.white))),
            ]
          )
        ) ?? false;
      }
      ...
      if (wantShort) {
        final picker = ImagePicker();
        final xfile = await picker.pickImage(source: ImageSource.camera, maxWidth: 800);
        if (xfile != null) {
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Analizando short con IA...')));
          final ai = ref.read(geminiServiceProvider);
          final jsonStr = await ai.analyzeGarment(name: 'Short de ${garment.name}', photoPath: xfile.path);
          ...
          if (jsonStr != null) {
            ...
            shortGarment = WardrobeGarment(
              id: '', name: 'Short de ${garment.name}',
              type: GarmentType.shorts,
              primaryColor: decoded['primaryColor'] as String? ?? '#000000',
              style: GarmentStyle.sport, material: decoded['material'] as String? ?? garment.material,
              season: garment.season, addedAt: DateTime.now()
            );
          } else {
            shortGarment = WardrobeGarment(
              id: '', name: 'Short de ${garment.name}', type: GarmentType.shorts, primaryColor: '#000000', style: GarmentStyle.sport, addedAt: DateTime.now()
            );
          }
          final savedShort = await ref.read(armarioProvider.notifier).addGarment(shortGarment);
          
          final outfit = Outfit(
            id: '', name: 'Conjunto ${garment.name}',
            garmentIds: [savedTop.id, savedShort.id],
            occasion: 'sport', season: garment.season,
            createdAt: DateTime.now(),
          );
          await ref.read(armarioProvider.notifier).saveOutfit(outfit);
          if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('¡Conjunto deportivo creado! ✨'), backgroundColor: Color(0xFF4CAF50)));
        }
      }
  ...
  Widget _field(TextEditingController c, String hint, IconData icon, {
    String? errorText, void Function(String)? onChanged,
      ...
      TextField(
        controller: c, style: TextStyle(color: Theme.of(context).colorScheme.onSurface), onChanged: onChanged,
  ...
  Widget _dropdown<T>(String label, IconData icon, T value, List<T> items,
    ...
    return DropdownButtonFormField<T>(
      ...
      items: items.map((v) => DropdownMenuItem(value: v,
          child: Text(display(v), style: TextStyle(color: Theme.of(context).colorScheme.onSurface)))).toList(),
      onChanged: onChanged,
    );
  }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('${WardrobeTab._typeEmoji[type] ?? '🎽'} ${type.label} (${items.length})', 
            style: TextStyle(color: Theme.of(context).colorScheme.onSurface, fontSize: 18, fontWeight: FontWeight.w600)),
      ),
      body: items.isEmpty 
        ? const Center(child: Text('No quedan prendas.', style: TextStyle(color: Colors.white38)))
        : GridView.builder(
            padding: const EdgeInsets.all(16),
            gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
              crossAxisCount: 2, mainAxisSpacing: 10,
              crossAxisSpacing: 10, childAspectRatio: 0.8,
            ),
